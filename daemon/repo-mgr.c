    ...
    while (1) {
        ...
        case WT_EVENT_DELETE:
            if (check_locked_file_before_remove (fset, event->path)) {
                ...
                try_add_empty_parent_dir_entry (repo->worktree, istate, event->path);
            }
            break;
...
/*
 * Strip the worktree path from the beginning of @full_path
 */
static char *
repo_relative_path (const char *worktree, const char *full_path)
{
    const char *rel_path;

    rel_path = full_path + strlen(worktree);
    if (*rel_path == '/')
        ++rel_path;
        ...
        if (fdata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            ...
            if (!DeleteFileW (sub_path_w)) {
                ...
                if (error == ERROR_SHARING_VIOLATION) {
                    *locked_files = g_list_prepend (*locked_files,
                                                    repo_relative_path(worktree,
                                                                       sub_path));
                }
...
static void
delete_worktree_dir (const char *worktree, const char *path, GList **locked_files)
{
    ...
    wchar_t *full_path_w = g_utf8_to_utf16 (full_path, -1,
                                            NULL, NULL, NULL);
    delete_worktree_dir_recursive_win32 (worktree, full_path_w, locked_files);
    g_free (full_path_w);
#else
...
#ifdef WIN32
            GList *locked_files = NULL, *ptr;
            char *locked_file;

            delete_worktree_dir (worktree, de->name, &locked_files);

            for (ptr = locked_files; ptr; ptr = ptr->next) {
                locked_file = ptr->data;

                ce = index_name_exists (&istate, locked_file, strlen(locked_file), 0);
                if (!ce)
                    continue;
...
#else
            delete_worktree_dir (worktree, de->name, NULL);
#endif
